...
children.push(
  makeTable(
    ...
    [
      ...
      ["F-14", "다중 QR 일괄 파싱", "한 이미지에서 여러 QR을 인식해 자동 매칭", "계획(Phase 4)"],
      ["F-15", "측정 이력 DB 저장", "SQLite 기반 측정 기록/이미지 메타 저장 (PostgreSQL 이관 가능 설계)", "계획(Phase 4)"],
      ["F-16", "이력 조회 화면", "QR ID·기간·Probe Type 필터로 과거 측정 결과 검색", "계획(Phase 4)"],
      ["F-17", "생산량 대시보드", "일/주/월/분기/연 단위 생산량 집계 및 차트", "계획(Phase 4)"],
      ["F-18", "DB Export/Import", "다른 PC와의 데이터 동기화를 위한 JSON/SQL 덤프 내보내기·가져오기", "계획(Phase 4)"],
    ]
  )
);
children.push(Blank());
...
children.push(
  makeTable(
    ...
    [
      ...
      ["확장성", "1차 목표: 단일 PC 설치형 / 다중 PC는 DB Export·Import로 대응 / 중장기 PostgreSQL 이관 경로 확보"],
      ["데이터 보관", "측정 이력 영구 보관 · 이미지는 파일시스템에 연/월 단위 분할 저장"],
    ]
  )
);
children.push(Blank());
